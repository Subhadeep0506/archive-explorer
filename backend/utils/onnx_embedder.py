"""
Shared ONNX SciBERT sentence embedder for KeyBERT keyword extraction.
...
config/vocab (~1 MB) is fetched from HuggingFace on first run — no model
...
_SCIBERT_TOKENIZER = "allenai/scibert_scivocab_uncased"


class ONNXSentenceEmbedder(BaseEmbedder):
    """
    KeyBERT-compatible embedder that runs a BERT-style ONNX model locally
    and mean-pools token embeddings into fixed-size sentence vectors.

    Subclasses BaseEmbedder so KeyBERT uses it directly without trying to
    load a HuggingFace model.

    For Q4F16 models, SimplifiedLayerNormFusion is automatically disabled
    to avoid conflicts with MatMulNBits ops.
    """

    def __init__(self, onnx_path: Path, tokenizer_name: str = _SCIBERT_TOKENIZER):
        super().__init__()
    ...
    def embed(self, documents: List[str], verbose: bool = False) -> np.ndarray:
        """Embed a list of documents into sentence vectors via ONNX inference."""
